...
echo -e "${redColour}\n\n[!]Updating system, please wait... This will take a while so why not go for a snack or pet your pets?"
...
go build -o degoded decoded64/main.go #Path updated 
if [ $? -ne 0 ]; then
...
sudo mv degoded /usr/local/bin/  # ✅ Nombre actualizado

# Dar permisos de ejecución
sudo chmod +x /usr/local/bin/degoded  # ✅ Nombre actualizado
...
# Compilar el programa
echo -e "${yellowColour}[+] Compiling degoded...${endColour}"
go build -o degoded decoded64/main.go  # ✅ Path actualizado
if [ $? -ne 0 ]; then
    echo -e "${redColour}[!] Error! Try again${endColour}"
    exit 1
fi
